...
export default function MainOrderView({ onNavigate, settingsOpen, onCloseSettings, onOpenSub, onLogOut, onDevModeChange, onOpenAlerts, stationCourse: stationCourseProp, historyCategories = [], historyCenters = [], onClearHistoryCategories, onClearHistoryCenters, onSetHistoryCategories, onSetHistoryCenters, cardVariant = 'default', legacyActions = false }: MainOrderViewProps) {
  ...
  const [viewMode, setViewMode] = useState<ViewMode>(staggerMode ? 'stagger' : 'horizontal');
  ...
  return (
    ...
    <div
      ...
      style={{ ...ticketSkinVars(resolveTicketSkin(theme)), paddingTop: 'calc(var(--training-bar-h, 0px) + var(--kds-header-h, 44px))' }}
    >
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        {settingsOpen ? (
          <div
            className="flex flex-1 overflow-hidden pl-0 pr-2 py-2 gap-2 bg-transparent"
            style={{ order: 2 }}
          >
            <div
        ...
        <div ref={boardContentRef} style={{ order: 2 }} className={`flex-1 flex flex-col overflow-hidden relative bg-tickets-bg ${getKdsScaleClasses(effectiveTextSize, effectiveTicketSpacing)}`}>
          {isHistory ? (
            <>
              {/* History filter bar */}
