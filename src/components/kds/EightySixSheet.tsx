...
const formatTimeRemaining = (endTime: Date | null, tui: (s: string, vars?: Record<string, unknown>) => string) => {
  if (!endTime) return tui("Until restored");
  const now = new Date();
  const diff = endTime.getTime() - now.getTime();
  if (diff <= 0) return tui("Expired");
  const minutes = Math.floor(diff / (1000 * 60));
  const hours = Math.floor(minutes / 60);
  if (hours > 0) return tui("{h}h {m}m left", { h: hours, m: minutes % 60 });
  return tui("{m}m left", { m: minutes });
};
  ...
  return (
    ...
    >
      <button
        ...
        aria-label={tui("Decrease Quantity")}
      >
      ...
      <button
        ...
        aria-label={tui("Increase Quantity")}
      >
...
const getPresetLabel = (preset: DurationPreset, tui: (s: string, vars?: Record<string, unknown>) => string): string => {
  if (preset === "indefinite") return tui("Until manually restored");
  if (preset === "15min") return tui("15 minutes");
  if (preset === "1hr") return tui("1 hour");
  if (preset === "end_of_shift") return tui("End of shift");
  const mins = Number(preset.split("-")[1]);
  if (mins >= 60) {
    const h = Math.floor(mins / 60);
    const m = mins % 60;
    return m ? tui("{h}h {m}m", { h, m }) : (h > 1 ? tui("{h} hours", { h }) : tui("{h} hour", { h }));
  }
  return tui("{n} minutes", { n: mins });
};
...
const getMinTimeRemainingLabel = (items: EightySixedItem[], tui: (s: string, vars?: Record<string, unknown>) => string): string => {
  const timed = items.filter((i) => i.snoozeEndTime);
  if (timed.length === 0) return tui("Until manually restored");
  const earliest = timed.reduce((min, i) =>
    (i.snoozeEndTime!.getTime() < min.snoozeEndTime!.getTime() ? i : min),
  );
  return formatTimeRemaining(earliest.snoozeEndTime, tui);
};
...
function RestorePopover(props: RestorePopoverProps) {
  ...
  const {
    ...
    triggerLabel = tui("Restore"), triggerClassName,
  } = props;
        ...
        {showCustomTimePicker ? (
          <div className="p-3">
            <div className="flex items-center gap-2 mb-3">
              ...
              <p className="text-foreground text-sm font-medium">{tui("Custom Time")}</p>
            </div>
            ...
            <Button onClick={onConfirmCustom} disabled={customHours === 0 && customMinutes === 0}
              className="w-full py-2 rounded-lg text-sm font-semibold">{tui("Confirm")}</Button>
          </div>
        ) : (
          <div className="py-2">
            <p className="text-muted-foreground text-xs px-3 py-1.5">{tui("Restore After")}</p>
...
function ManageEightySixedList(props: ManageListProps) {
  ...
  const renderItemSubtext = (item: EightySixedItem) => {
    ...
    const timeLabel = item.snoozeEndTime ? formatTimeRemaining(item.snoozeEndTime, tui) : getPresetLabel(preset, tui);
    if (stock === "partial") {
      return (
        <p className="text-xs text-muted-foreground mt-0.5">
          <span className="text-amber-600 dark:text-amber-400 font-semibold">{tui("{n} left", { n: item.quantity })}</span>
          <span> · {timeLabel}</span>
        </p>
      );
    }
    return <p className="text-xs text-muted-foreground mt-0.5">{tui("Out of stock")} · {timeLabel}</p>;
  };
    ...
    <div className="pb-4">
      ...
      <div className="px-4 pt-3 pb-2">
        <div className="flex items-center justify-between gap-3">
          <p className="text-foreground text-base font-bold">{tui("{n} items 86'd", { n: items.length })}</p>
          <RestorePopover {...props} bulkItems={items} bulkKey="all" triggerLabel={tui("Restore all")} />
        </div>
      </div>
            ...
            <div key={category} className="border-t border-border">
              <div className="flex items-center justify-between px-4 py-2 bg-muted/40">
                ...
                <RestorePopover
                  ...
                  bulkItems={list}
                  bulkKey={category}
                  triggerLabel={tui("Restore all ({n})", { n: list.length })}
...
export function EightySixSheet({
  ...
  const activeDurationLabel =
    tui(snoozeDurations.find((d) => d.id === selectedDuration)?.label ?? "1 hour");
    ...
    <Sheet open={open} onOpenChange={handleSheetOpenChange}>
      ...
      >
            ...
            <div className="flex items-center gap-1.5">
              ...
              {view === "add" && (
                <button
                  ...
                  aria-label={tui("Search Products")}
          ...
          <ScrollArea className="flex-1 w-full">
            {eightySixedItems.length === 0 ? (
              <div className="absolute inset-0 flex flex-col items-center justify-center px-4 text-center">
                ...
                <p className="text-muted-foreground text-center w-full">{tui("No products are currently 86'd")}</p>
                <Button
                  onClick={() => setView("add")}
          ...
          <>
              ...
              <div className="px-4 py-2 border-b border-border">
                <div className="relative">
                  ...
                  <Input
                    ...
                    placeholder={tui("Search menu products...")}
            ...
            <div className="flex items-center justify-between px-4 py-2 border-b border-border gap-3">
              <div className="flex items-center gap-2 text-sm text-foreground">
                ...
                <span>{tui("Mark Unavailable for")}</span>
              </div>
              ...
              return (
                <div className="flex items-center justify-between px-4 py-2 border-b border-border">
                  <span className="text-xs text-muted-foreground">
                    {selectedItems.size > 0 ? tui("{n} selected", { n: selectedItems.size }) : tui("Tap the product to select")}
                  </span>
                  <button
                    type="button"
                  ...
                  >
                    {allSelected ? tui("Deselect All") : tui("Select All")}
                  </button>
                </div>
              );
            })()}
                    ...
                    <div key={category.name}>
                      <div className="flex items-center justify-between px-1 py-1">
                        ...
                        {category.items.some((i) => selectedItems.has(itemKey(i, category.name))) && (
                          <span className="text-xs text-muted-foreground">{tui("Available Stock")}</span>
                        )}
                      </div>
        ...
        <Item86Modal
          ...
          currentQuantity={0}
          initialQuantity={0}
          quantityLabel={tui("Available stock")}
          renderInPlace
        />
        ...
        {view === "add" && selectedItems.size > 0 && (
          <div className="absolute bottom-4 right-4 z-50">
            ...
            >
              {tui("86 it ({n})", { n: selectedItems.size })}
            </Button>
          </div>
        )}
