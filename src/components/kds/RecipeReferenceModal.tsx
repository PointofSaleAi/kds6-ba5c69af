...
export function RecipeReferenceModal({ product, order, courseLabel, onClose, variant = 'default' }: Props) {
  ...
  useEffect(() => {
    ...
    const u = m0 ? m0[1] : 'portion';
    const has = base.yields.some((y) => {
      const m = y.match(/^(\d+)/);
      return m ? parseInt(m[1], 10) === product.quantity : false;
    });
    const list = has ? base.yields : [`${product.quantity} ${u}`, ...base.yields];
    const idx = list.findIndex((y) => {
      const m = y.match(/^(\d+)/);
      return m ? parseInt(m[1], 10) === product.quantity : false;
    });
              ...
              {recipe.ingredients.length > 0 && (
                <section style={{ marginBottom: 8 }}>
                  <div className="flex items-center justify-between" style={{ marginBottom: 8 }}>
                    ...
                    <div style={{ width: 160, fontSize: S.sectionLabel, color: C.textMuted, textTransform: 'uppercase', letterSpacing: 1.2, fontWeight: 600, textAlign: 'left' }}>
                      Full recipe
                    </div>
                  </div>
                  ...
                  <div className="flex" style={{ gap: 8 }}>
                    ...
                    <div
                      ...
                      style={{ width: 160, background: C.surfaceSubtle, border: `1px solid ${C.border}`, alignSelf: 'stretch' }}
                    >
