    ...
    <>
      ...
      <div className="fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 z-50 w-[680px] max-w-[95vw] bg-surface-card rounded-[20px] shadow-2xl overflow-hidden">
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-border">
          <h3 className="text-base font-bold text-text-primary">{tui('Reply to Message')}</h3>
          <button
            onClick={onClose}
        ...
        <div className="px-5 py-3 bg-muted/30 border-b border-border">
          <p className="text-xs font-semibold text-text-secondary mb-1">
            {tui('From {name}', { name: message.terminal_name || message.employee_name })}
          </p>
        ...
        <div className="flex divide-x divide-border">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="px-5 pt-4 pb-2">
              <p className="text-xs font-semibold text-text-secondary mb-2">{tui('Quick Replies')}</p>
          ...
          <div className="w-[220px] flex flex-col items-center justify-center px-4 py-4 gap-3">
            ...
            {expired ? (
              ...
              <p className="text-[11px] text-text-muted font-mono tabular-nums">
                {tui('Expires in {m}:{s}', { m: mins, s: secs.toString().padStart(2, '0') })}
              </p>
            )}
          </div>
        </div>
