  ...
  return (
    ...
    >
      <div className="flex items-start" style={{ gap: 'var(--kds-item-gap)' }}>
        <span className="font-medium shrink-0 text-center text-text-muted" style={{ fontSize: 'var(--kds-item-qty)', minWidth: 20, lineHeight: 1.2 }}>
          {product.quantity}
        </span>
...
export function OrderCardV4({ order, onBump, onMarkSeen, onItemDone, onItemDismiss, isSeen }: Props) {
            ...
            {order.courses.map((course, idx) => {
              ...
              return (
                <div key={`${course.course}-${idx}`}>
                  <div
                    className="px-2 py-1 text-[11px] font-medium uppercase tracking-wide text-text-secondary"
                    style={{ background: 'var(--tkt-chip-bg)' }}
                  >
